...
! defined( 'WOOCO_VERSION' ) && define( 'WOOCO_VERSION', '4.1.7' );
		...
		if ( ! class_exists( 'WPCleverWooco' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWooco {
                                    ...
                                    <div class="wooco_component_content_line">
                                        ...
                                        <div class="wooco_component_content_line_value">
                                            ...
                                            <div class="wooco_hide wooco_show_if_tags">
                                                <input class="wooco-category-search-input wooco_component_tags_val"
				...
				function wooco_order_formatted_line_subtotal( $subtotal, $item ) {
					if ( ! empty( $item['wooco_ids'] ) && ! empty( $item['wooco_price'] ) ) {
					...
					if ( isset( $cart_item['wooco_ids'], $cart_item['wooco_keys'] ) && method_exists( $cart_item['data'], 'get_pricing' ) && ( $cart_item['data']->get_pricing() !== 'only' ) ) {
						...
						foreach ( $cart_item['wooco_keys'] as $cart_item_key ) {
							if ( isset( WC()->cart->cart_contents[ $cart_item_key ] ) ) {
								if ( isset( WC()->cart->cart_contents[ $cart_item_key ]['woosb_ids'], WC()->cart->cart_contents[ $cart_item_key ]['woosb_price'] ) ) {
									// WPC Product Bundles
									$price += wc_get_price_to_display( WC()->cart->cart_contents[ $cart_item_key ]['data'], array(
										'qty'   => WC()->cart->cart_contents[ $cart_item_key ]['wooco_qty'],
										'price' => WC()->cart->cart_contents[ $cart_item_key ]['woosb_price']
									) );
								} else {
				...
				function wooco_before_calculate_totals( $cart_object ) {
					...
					foreach ( $cart_contents as $cart_item_key => $cart_item ) {
						...
						if ( ! empty( $cart_item['wooco_parent_key'] ) ) {
							...
							if ( ( $parent_new_key === $parent_key ) && isset( $cart_contents[ $parent_key ]['wooco_keys'] ) && ! in_array( $cart_item_key, $cart_contents[ $parent_key ]['wooco_keys'] ) ) {
								unset( $cart_contents[ $cart_item_key ] );
								continue;
							}
								...
								if ( $cart_item['data']->get_pricing() !== 'only' ) {
									...
									foreach ( $cart_item['wooco_keys'] as $key ) {
										if ( isset( $cart_contents[ $key ] ) ) {
											...
											if ( ! empty( $cart_contents[ $key ]['wooco_price'] ) ) {
					...
					if ( $components = $product->get_components() ) {
                        ...
                        <div class="wooco_components wooco-components"
							...
							foreach ( $components as $component ) {
								...
								$component_products   = $this->wooco_get_products( $component_type, $component_orderby, $component_order, $component[ $component_type ], $component_exclude, $component_default, $component_qty, $component_price, $component_custom_qty );
									...
									if ( $only_product ) {
                                        ...
                                        <div class="wooco_component_product wooco_component_product_only"
											...
											<?php if ( $component_custom_qty ) {
												...
												$qty_input .= '</span><!-- /wooco-qty-input -->';
												$qty_input .= '</div><!-- /wooco-qty -->';
				...
				function wooco_get_products( $type, $orderby, $order, $data, $exclude = '', $default = 0, $qty = 1, $price = '', $custom_qty = false ) {
					$has_default = false;
					$products    = array();
					$ids         = explode( ',', $data );
						...
						case 'products':
							...
							foreach ( $ids as $id ) {
								...
								if ( $_product->is_type( 'simple' ) || $_product->is_type( 'variation' ) || $_product->is_type( 'woosb' ) ) {
									...
									$products[ 'pid_' . $id ] = $this->wooco_get_product_data( $_product, $qty, $price, $custom_qty );
								}
								...
								if ( $_product->is_type( 'variable' ) ) {
									...
									if ( ! empty( $children ) ) {
										foreach ( $children as $child ) {
											...
											if ( ! $child_product || ! $child_product->variation_is_visible() || ( ( get_option( '_wooco_exclude_unpurchasable', 'yes' ) === 'yes' ) && ! $this->wooco_is_purchasable( $child_product, $qty ) ) ) {
												continue;
											}
				...
				function wooco_get_product_data( $product, $qty = 1, $price = '', $custom_qty = false ) {
					...
					$_price_display = wc_get_price_to_display( $product );
